...
const SuggestTags = props => {
  ...
  const input = useRef(); // filter suggestions on tags change
  ...
  const hadnleKeyUp = e => {
    ...
    const isUpOrDownKey = e.key === UP_ARROW || e.key === UP_ARROW_COMPAT || e.key === DOWN_ARROW || e.key === DOWN_ARROW_COMPAT;
    if (isUpOrDownKey) return; // else continue
  ...
  const removeTag = index => {
    ...
    setTags(newSet);
    onTagRemove(newSet, value, index); // setSelectedSuggestionIndex(0)
  };
  ...
  const filterSuggestions = (searchValue = '') => {
    ...
    setSuggestions(suggestionsPool.filter((el, i) => {
      ...
      if (counter >= maxSuggestionLength) return false; // terminate early if already in tags

      let inTags = tags.indexOf(lowercase ? el.toLowerCase() : el) !== -1;
      if (inTags) return false; // filter out elements that contain searchTerm (case insensitive)
  ...
  }, tags.map((tag, i) => /*#__PURE__*/React.createElement("button", {
  ...
  }), expanded && suggestions.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("ul", {
    className: classNames.ul
  }, suggestions.map((item, i) => /*#__PURE__*/React.createElement("li", {
    key: i,
    onClick: e => addTag(item),
    onMouseDown: handleSuggestionMouseDown,
    className: classNames.li
  }, item))), /*#__PURE__*/React.createElement("style", null, getStylesForActiveSuggestion(selectedSuggestionIndex)))));
};
